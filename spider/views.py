...
from requests import ReadTimeout, Response, RequestException
...
class PtSpider:
    ...
    def get_userinfo_html(self, my_site: MySite):
        """请求抓取数据相关页面"""
        site = get_object_or_404(WebSite, id=my_site.site)
        ...
        if site.url in [
            'greatposterwall', 'dicmusic'
        ]:
            details_html = user_detail_res.json()


def get_seeding_html(self, my_site: MySite):
    """请求做种数据相关页面"""
    site = get_object_or_404(WebSite, id=my_site.site)
