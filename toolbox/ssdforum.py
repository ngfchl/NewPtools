...
def sign_ssd_forum(cookie, user_agent, todaysay):
    try:
        ...
        print(f'签到页HTML：{sign_response.text}')
        html_object = etree.HTML(sign_response.text)
        sign_check = html_object.xpath('//span[@class="gray"]')
        print(f"签到检测：{sign_check}")
        sign_text = ''
        if sign_check or len(sign_check) > 0:
            print(f"签到检测：{sign_check}")
            ...
            # 解析签到反馈
            print(f'签到反馈：{sign_in_response.text}')
            sign_text = ''.join(etree.HTML(sign_in_response.text).xpath('//div[@class="c"]/text()'))
        else:
            sign_text = '今日已签到'
            print(sign_text)
        sign_response = requests.get(
        ...
        print(f"签到页：{sign_response.text}")
        sign_title_rule = '//div[@class="mn"]/h1[1]/text()'
        sign_content_rule = '//div[@class="mn"]/p//text()'
        title = etree.HTML(sign_response.text).xpath(sign_title_rule)
        content = etree.HTML(sign_response.text).xpath(sign_content_rule)
